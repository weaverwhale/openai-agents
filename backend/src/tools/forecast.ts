...
export const generateForecast = async (
...
): Promise<{ forecast: string } | { error: string }> => {
  ...
  try {
    ...
    result += `   • Trend: ${
      linearModel.slope > 0.1 ? 'Increasing' : linearModel.slope < -0.1 ? 'Decreasing' : 'Stable'
    }\n`;
...
export const forecastTool = tool({
  name: 'forecast',
  description:
    'Generate forecasts and predictions based on time series data using statistical analysis. Useful for predicting trends, sales forecasting, and data analysis.',
  parameters: z.object({
    data: z
      .array(z.number())

...
export const lookupUrbanDictionary = async (term: string): Promise<{ definition: string } | { error: string }> => {
  if (!term) {
...
export const urbanDictionaryTool = tool({
  ...
  description: 'Look up slang definitions and internet culture terms from Urban Dictionary. Useful for understanding modern slang, internet terminology, and pop culture references.',
  parameters: z.object({
  ...
  execute: async ({ term }) => {
    try {
      ...
      return `Error looking up Urban Dictionary: ${error instanceof Error ? error.message : 'Unknown error occurred'}`;
    }
  },
}); 